"""TD3 training with validation-2024-only checkpoint selection."""
...
def evaluate(agent: TD3Agent, env: SACIrrigationEnv):
...
def _snapshot(agent: TD3Agent) -> dict:
    return {
...
def _restore(agent: TD3Agent, state: dict) -> None:
...
def train(
    ...
    set_seed(seed)
    noise_rng = np.random.default_rng(seed + 90_000)
    replay = ReplayBuffer(100_000, env.observation_dim, seed)
    history, selections, total_steps = [], [], 0
    ...
    for episode in range(1, episodes + 1):
        ...
        history.append({
            ...
            **{
                key: float(value.cpu()) if isinstance(value, torch.Tensor) else value
